...
internal class BusinessContext : ContextBase
{
    ...
    public async Task<bool> PushEvent(ProtocolEvent @event)
    {
        try
        {
            ...
            foreach (var packet in packets) await Collection.Packet.PostPacket(Collection.AppInfo, packet);
        }
        catch
        {
            return false;
    ...
    public async Task<List<ProtocolEvent>> SendEvent(ProtocolEvent @event)
    {
        ...
        try
        {
            ...
            {
                var returnVal = await Collection.Packet.SendPacket(Collection.AppInfo, packet);
