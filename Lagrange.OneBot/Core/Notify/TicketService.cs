...
public class TicketService
{
    ...
    private readonly Dictionary<string, (string PsSey, DateTimeOffset ExpireTime)> _psKeys;

    private (string SKey, DateTimeOffset ExpireTime) _sKey;
    ...
    public TicketService(BotContext context)
    {
        ...
        _psKeys = new Dictionary<string, (string, DateTimeOffset)>();
    }
    ...
    {
        if (DateTimeOffset.Now < _sKey.ExpireTime) return _sKey.SKey;
        ...
        _sKey = (sKey, DateTimeOffset.Now.AddDays(1));
    ...
    public async Task<string> GetCookies(string domain)
    {
        ...
        {
            if (DateTimeOffset.Now > tokenTime.ExpireTime)
            {
                token = (await _context.FetchCookies([domain]))[0];
                _psKeys[domain] = (token, DateTimeOffset.Now.AddDays(1)); // update pskey
            }
            else
            {
        ...
        {
            token = (await _context.FetchCookies([domain]))[0];
            _psKeys[domain] = (token, DateTimeOffset.Now.AddDays(1)); // get pskey
        }
