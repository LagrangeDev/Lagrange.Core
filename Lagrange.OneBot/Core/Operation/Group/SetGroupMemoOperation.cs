...
{
    [Serializable]
    private class NoticeSendResponse
    {
        [JsonPropertyName("new_fid")] public string NoticeId { get; set; } = string.Empty;
    }

    [Serializable]
    private class NoticeImageUploadResponse
    {
    ...
    private const string _url = "https://web.qun.qq.com/cgi-bin/announce/add_qun_notice?bkn=";
    private const string _userAgent = "Dalvik/2.1.0 (Linux; U; Android 7.1.2; PCRT00 Build/N2G48H)";

    private async Task<string?> SendGroupNoticeSimple(OneBotSetGroupMemo memo)
    {
        var bkn = await ticket.GetCsrfToken();
        var body = $"qid={memo.GroupId}&bkn={bkn}&text={Uri.EscapeDataString(memo.Content)}&pinned=0&type=1&settings={{\"is_show_edit_card\":0,\"tip_window_type\":1,\"confirm_required\":1}}";
        var url = _url + bkn;
        try
        {
